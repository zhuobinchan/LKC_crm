		...
		<div class="row">
			...
			<div class="span9">
				...
				<ul class="files zebra-list" id="itemContainer">
					<c:forEach items="${allcontract }" var="allcontract">
						<li><i class="icon-file"></i> <a class="title"
							href="${pageContext.request.contextPath }/servlet/search_pdf_servlet?filename=${allcontract.contract_name }">${allcontract.contract_name}</a>
							<span class="meta">Uploaded <em>${allcontract.contract_time }</em> by
								<em>${allcontract.contract_user}</em></span></li>
					</c:forEach>
				</ul>
				...
				<form id="new-file" class="form-horizontal hidden"
					action="${pageContext.request.contextPath }/servlet/updateContractServlet"
