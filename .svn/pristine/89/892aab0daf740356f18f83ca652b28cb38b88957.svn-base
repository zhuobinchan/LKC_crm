...
public class updateFileServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*InputStream in = request.getInputStream();//获取请求中的数据流
		String tempfileload = "E:/tempdata1";//临时文件位置
		
		File tempfile = new File(tempfileload);//tempfile指向临时文件
		FileOutputStream out = new FileOutputStream(tempfile);//通过输出流向临时文件写入数据
